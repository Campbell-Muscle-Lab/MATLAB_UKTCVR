...
% Function creates a reproducible panel layout
    ...
    arguments
        ...
        options.panel_label_font_size (1,1) double = 14
    ...
    for row = 1 : options.panels_high
        ...
        if (row == 1)
            axis_width = (options.figure_width - ...
                            ...
                            0*sum(options.padding_left_adjustments(row_indices)) - ...
                            sum(options.padding_right(row_indices)) + ...
                            0*sum(options.padding_right_adjustments(row_indices))) / ...
                        options.panels_wide;
        ...
        for col = 1 : options.panels_wide
            ...
            if (options.panel_label) & (options.panel_label_strings(subplot_counter) ~= "")
                ...
                x_label = -options.padding_left(subplot_counter) - ...
                            options.padding_left_adjustments(subplot_counter);
...
% Fills out z so that its length is equal to the number of panels
    ...
    % Check if arrays are correct length
    if (numel(z) == no_of_panels)
        % Nothing to do
        return
    end
