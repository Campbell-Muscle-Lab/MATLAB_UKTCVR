    ...
    for file_i = 1 : numel(data_file_strings)
        ...
        for panel_i = 1:numel(template.panels)
            ...
            for series_i = 1 : numel(series_data)
                ...
                % Check for ticks
                panel_data = template.panels(panel_i).y_info;
                if (isfield(panel_data, 'ticks'))
                    if (~ismissing(panel_data.ticks))
                        y(y<panel_data.ticks(1)) = NaN;
                        y(y>panel_data.ticks(end)) = NaN;
                    end
                end
                ...
                switch (formatting.style)
                    case 'reference'
                        ...
                        if ( (isfield(series_data(series_i), "x_limits")) && ...
                                (~any(ismissing(series_data(series_i).x_limits))) )
                            % Over-ride defaults
                            x_lim = series_data(series_i).x_limits;
                            ti = [find(x_all >= x_lim(1), 1, 'first') ...
                                    find(x_all <= x_lim(end), 1, 'last')];
                            x_ref = x_all(ti);
                        end
                    ...
                    case 'envelope'
                        [x_env, y_env] = return_envelope( ...
                            x_all(line_ti), y(line_ti), ...
                            formatting.envelope_no_of_bins, options);
                ...
                h_leg(panel_i).legend_title = "";
                if ( (isfield(template.panels(panel_i).y_info, "legend_title")) && ...
                        (~ismissing(template.panels(panel_i).y_info.legend_title)) )
                    h_leg(panel_i).legend_title = ...
                        template.panels(panel_i).y_info.legend_title;
                end
            end
        end
    end
...
function [x_env, y_env] = return_envelope(x_all, y_all, ...
    envelope_no_of_bins, options)
...
x_bins = NaN * ones(envelope_no_of_bins, 1);
y_top = NaN * ones(envelope_no_of_bins, 1);
y_bottom = NaN * ones(envelope_no_of_bins, 1);

x_edges = linspace(x_min, x_max, (envelope_no_of_bins+1));
        ...
        for pan_i = 1 : numel(panel_list)
            ...
            switch an.type
                ...
                case "vline"
                    ...
                    if ( (isfield(an, "color")) && ...
                            (all(~ismissing(an.color))) )
                        an.color = an.color;
                    else
                        an.color = options.vline_color;
                    end
                    ...
                    if ( isfield(an, "label") && ~ismissing(an.label) )
                        ...
                        an.horizontal_alignment = "center";
                        if (isfield(an, "horizontal_alignment") && ...
                                (~ismissing(an.horizontal_alignment)) )
                            an.horizontal_alignment = an.horizontal_alignment;
                        end
                        ...
                        text(x, y, ...
                            ...
                            FontSize = options.annotation_font_size, ...
                            Color = an.color, ...
                            HorizontalAlignment = an.horizontal_alignment, ...
                            VerticalAlignment = 'middle', ...
                            Clipping = 'off')
                    end
                ...
                case "text"
                    ...
                    if ( (isfield(an, "color")) && ...
                        ...
                        an.color = options.annotation_font_color;
                    end
